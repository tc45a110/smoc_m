...
{
	private static Logger logger = Logger.getLogger(SMGPConstant.class);
  /**存放系统是否为调试状态*/
...
/**错误常量-正在登录*/
  ...
  //定义协议中用到Command_ID的值
	...
	static{
		try {
			resource = new Resource(new SMGPConstant().getClass(), "resource");
		} catch (IOException e) {
			e.printStackTrace();
		}
		initConstant();
     ...
     * 系统初始化时调用，读取常量资源字符串。
     */
    private synchronized static void initConstant()
    {
    	logger.info("常量资源初始化!");
        if(LOGINING == null)
        {
