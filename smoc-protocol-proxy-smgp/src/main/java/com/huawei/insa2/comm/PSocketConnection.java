...
public abstract class PSocketConnection{
	...
	protected BlockingQueue<SMGPMessage> sendSubmitRespQueue;
	...
	protected class ConnectionTask extends TimerTask {
		...
		private int key;
		private long time;
		private SMGPMessage message;

		public ConnectionTask(int key,SMGPMessage message) {
			this.key = key;
		...
		public void run() {
			try {
				taskMap.remove(key);
				CategoryLog.connectionLogger.warn("连接无响应sequence=" + key+",command="+Integer.toHexString(message.getRequestId()));
			} catch (Exception e) {
	 ...
	 * 初始化操作。
	 */
	protected void init(BlockingQueue<SMGPMessage> sendSubmitRespQueue) {
	 ...
	 *            发送的消息。
	 */
	public void send(SMGPMessage message){
	...
	protected synchronized void connect() {
		...
		try {
			request = new SMGPLoginMessage(source_addr, version, shared_secret, loginMode, new Date());
	...
	public void onReceive(SMGPMessage message) {
		int sequence = message.getSequenceId();
		int requestId = message.getRequestId();
		CategoryLog.connectionLogger.info("接收:CommandId={},sequence={},localPort={}",Integer.toHexString(requestId),sequence,localPort);
		//Submit_Rep_Command_Id、Connect_Rep_Command_Id、Active_Test_Rep_Command_Id只需要接收
		if(requestId == SMGPConstant.Submit_Resp_Request_Id ){
			this.sendSubmitRespQueue.add(message);
		}else if(requestId == SMGPConstant.Login_Resp_Request_Id || requestId == SMGPConstant.Active_Test_Request_Id ){
			this.connectionSubmitRespQueue.add(message);
		}else{	//需要写响应数据
			SMGPMessage responseMessage = null;
			//状态报告
			if(requestId == SMGPConstant.Deliver_Request_Id){
				responseMessage = proxy.onDeliver(message);
			}else if(requestId == SMGPConstant.Active_Test_Request_Id){
