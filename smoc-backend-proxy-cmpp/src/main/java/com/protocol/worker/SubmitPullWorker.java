...
import com.protocol.proxy.util.CmppUtil;

public class SubmitPullWorker extends SuperQueueWorker<CMPPMessage>{
	 ...
	 */
	private void init(){
		if(ChannelInfoManager.getInstance().isCMPP20Version(channelID)){
			proxy = new SMProxy(channelID, index, superQueue,reportWorker);
		}else{
			proxy = new SMProxy30(channelID, index, superQueue,reportWorker);
		}
	}
	...
	protected void doRun() throws Exception {
		...
		if(proxy.isHealth() && !responseWorker.overGlideWindow()){
			...
			if(businessRouteValue != null){
				...
				CMPPMessage[] messages = CmppUtil.packageSubmit(ChannelInterfaceUtil.getArgMap(channelID), businessRouteValue.getMessageContent(), businessRouteValue.getPhoneNumber(),channelSubmitSRCID,ChannelInfoManager.getInstance().isCMPP20Version(channelID),businessRouteValue.getMessageFormat());
