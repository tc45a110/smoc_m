...
public class AccountChannelPrivilegeController {
    ...
    public ModelAndView list() {
        ...
        configChannelChangeValidator.setChangeType("PRIVILEGE");
    ...
    public ModelAndView add(@PathVariable String channelId, HttpServletRequest request) {
        ...
        ModelAndView view = new ModelAndView("configure/account_channel_change/account_channel_privilege_add");
    ...
    public ModelAndView Save(@ModelAttribute @Validated ConfigChannelChangeValidator configChannelChangeValidator, BindingResult result, @PathVariable String op, HttpServletRequest request) {
        ...
        //保存操作记录
        if (ResponseCode.SUCCESS.getCode().equals(data.getCode())) {
            systemUserLogService.logsAsync("CHANNEL_CHANGE", configChannelChangeValidator.getId(), user.getRealName(), "PRIVILEGE", "通道特权", JSON.toJSONString(configChannelChangeValidator));
        }

        //记录日志
        log.info("[通道切换][通道特权][{}][{}]数据:{}", op, user.getUserName(), JSON.toJSONString(configChannelChangeValidator));


        view.setView(new RedirectView("/configure/channel/privilege/list", true, false));
    ...
    public ModelAndView record(@PathVariable String id, HttpServletRequest request) {
        ...
        ModelAndView view = new ModelAndView("configure/account_channel_change/account_channel_privilege_change_list");
