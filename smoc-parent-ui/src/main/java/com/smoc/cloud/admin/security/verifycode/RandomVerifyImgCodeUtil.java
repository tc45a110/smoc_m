...
public class RandomVerifyImgCodeUtil {
	...
	private static Logger logger = LoggerFactory.getLogger(RandomVerifyImgCodeUtil.class);
	...
	static {
		try {
			...
			logger.error("new img font font format failed. e:{} ", e.getMessage());
		} catch (IOException e) {
			logger.error("new img font io failed. e:{} ", e.getMessage());
		}
	}
	 ...
	 *            验证码
	 * @param type
	 *            场景类型，login：登录， coupons：领券 登录清晰化，领券模糊化 3D: 3D中空自定义字体 GIF：普通动态GIF
	 *            GIF3D：3D动态GIF mix2: 普通字体和3D字体混合 mixGIF: 混合动态GIF
	 * @throws IOException
	 */
	...
	static class ImgFontByte {
		...
		private String getFontByteStr() {
			...
			return new StringBuffer( 
					"0001000000100040000400c04f532f327d8175d4000087740000005650434c")
							.toString();
		}
	...
	public static void outputImage(int w, int h, File outputFile, String code) throws IOException {
		...
		try {
			...
			// outputImage(w, h, fos, code, "coupons"); //测试领券，噪点和干扰线为范围随机值0.05f ~ 0.1f和20 ~
			// 155
			...
			// outputImage(w, h, fos, code, "GIF3D"); //测试领券，噪点和干扰线为范围随机值0.05f ~ 0.1f和20 ~
			// 155
			// outputImage(w, h, fos, code, "mix2"); //测试领券，噪点和干扰线为范围随机值0.05f ~ 0.1f和20 ~
			// 155
