...
public class ReportTimerTaskWorkerManager extends SuperConcurrentMapWorker<String, TimerTask> {
	...
	private BlockingQueue<String> sequenceNumbers = new LinkedBlockingQueue<String>();
	...
	public void removeReportTaskByResponse(String sequenceNumber) {
		sequenceNumbers.add(sequenceNumber);
	}

	public void addReportTimeoutTask(Report report,String sequenceNumber) {
		TimerTask responseTimeoutTask = new ResponseTimeoutTask(sequenceNumber,report);
		add(sequenceNumber, responseTimeoutTask);
	...
	protected void doRun() throws Exception {
		logger.info("等待状态报告响应数量:{},待匹配状态报告响应数量:{}",superMap.size(),sequenceNumbers.size());
	...
	class ReportTimerTaskWorker extends SuperCacheWorker {
		...
		@Override
		protected void doRun() throws Exception {
			String sequenceNumber = sequenceNumbers.take();
			logger.debug("待处理已接收响应的状态报告数{}:", sequenceNumbers.size());
			ResponseTimeoutTask responseTimeoutTask = (ResponseTimeoutTask) remove(sequenceNumber);
			if (responseTimeoutTask == null) {
				logger.info(new StringBuilder().append("未找到状态报告信息").append("{}sequenceNumber={}").toString(),
						FixedConstant.SPLICER, sequenceNumber);
				return;
			}
			logger.debug("删除状态报告信息{}sequenceNumber={},report={}",FixedConstant.SPLICER, sequenceNumber, responseTimeoutTask.report);
	...
	class ResponseTimeoutTask extends TimerTask {
		...
		public ResponseTimeoutTask(String sequenceNumber,Report report) {
			super();
