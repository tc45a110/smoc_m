package com.protocol.access.smpp.sms;
...
public class ShortMessage extends ByteData {
	...
	public String getMessage(String client) {
		//增加逻辑判断，需要按照7bit编码格式
		//String clients = "";
		boolean flag = BusinessDataManager.getInstance().isAccountSupport7bitCode(client);
		
//		if(clients != null && clients.contains(client)){
//			logger.info(client+"需要遵循7bit编码格式");
//			flag = true;
//		}
		String str = "";
		try {
			...
			if(tpUdhi == 1){
				...
				if(msgFormat == 4) {
					str = Base64.encodeBase64String(body);
				}else if(msgFormat == 0 && flag) {
					str = Tools.gsm7BitDecode(body);
				}else{
				...
				if(msgFormat == 4) {
					...
					str = Tools.gsm7BitDecode(messageData);
				}else{
