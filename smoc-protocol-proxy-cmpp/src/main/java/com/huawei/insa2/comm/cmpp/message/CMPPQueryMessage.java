...
public class CMPPQueryMessage extends CMPPMessage {
	...
	public CMPPQueryMessage(Date time, int query_Type, String query_Code,
		...
		if (query_Type != 0 && query_Type != 1)
			throw new IllegalArgumentException(String.valueOf(String
					.valueOf((new StringBuffer(String.valueOf(String
							.valueOf(CMPPConstant.QUERY_INPUT_ERROR)))).append(
							":query_Type").append(CMPPConstant.VALUE_ERROR))));
		if (query_Code.length() > 10)
			throw new IllegalArgumentException(String.valueOf(String
					.valueOf((new StringBuffer(String.valueOf(String
							.valueOf(CMPPConstant.QUERY_INPUT_ERROR))))
							.append(":query_Code")
		...
		if (reserve.length() > 8) {
			...
			System.arraycopy(dateFormat.format(time).getBytes(), 0, super.buf,
					12, dateFormat.format(time).length());
			super.buf[20] = (byte) query_Type;
			System.arraycopy(query_Code.getBytes(), 0, super.buf, 21,
					query_Code.length());
			System.arraycopy(reserve.getBytes(), 0, super.buf, 31,
					reserve.length());
			outStr = ",time=".concat(String.valueOf(String.valueOf(dateFormat
					.format(time))));
			outStr = String.valueOf(String.valueOf((new StringBuffer(String
					.valueOf(String.valueOf(outStr)))).append(",query_Type=")
					.append(query_Type)));
			outStr = String.valueOf(String.valueOf((new StringBuffer(String
					.valueOf(String.valueOf(outStr)))).append(",query_Code=")
					.append(query_Code)));
			outStr = String.valueOf(String.valueOf((new StringBuffer(String
					.valueOf(String.valueOf(outStr)))).append(",reserve=")
					.append(reserve)));
			return;
		}
	}
