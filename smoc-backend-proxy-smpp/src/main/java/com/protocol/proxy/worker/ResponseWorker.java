...
public class ResponseWorker extends SuperMapWorker<Integer, TimerTask>{
	...
	private BlockingQueue<SMPPMessage> responseQueue;
	
	

	ResponseWorker(String channelID,String index,BlockingQueue<SMPPMessage> responseQueue) {
	...
	protected void doRun() throws Exception {
		SMPPMessage message = responseQueue.poll(FixedConstant.COMMON_POLL_INTERVAL_TIME,TimeUnit.SECONDS);
		if(message == null){
			return;
		}
		...
		SMPPSubmitRespMessage responseMessage = (SMPPSubmitRespMessage) message;
		channelMessageID = responseMessage.getMessageId();
		nextNodeErrorCode = String.valueOf(responseMessage.getStatus());
