...
import com.protocol.proxy.util.SmppUtil;

public class SubmitPullWorker extends SuperQueueWorker<SMPPMessage>{
	private String channelID;
	/**
	 * 链接序号
	 */
	private String index;
	private SMPPSMProxy proxy;
	 ...
	 */
	private void init(){
		proxy = new SMPPSMProxy(channelID, index, superQueue,reportWorker);
	}
	...
	protected void doRun() throws Exception {
		...
		if(proxy.isHealth() && !responseWorker.overGlideWindow()){
			...
			if(businessRouteValue != null){
				...
				SMPPMessage[] messages = SmppUtil.packageSubmit(ChannelInterfaceUtil.getArgMap(channelID), businessRouteValue.getMessageContent(), businessRouteValue.getPhoneNumber(), channelSubmitSRCID, businessRouteValue.getAccountID());
