...
{
	private static Logger logger = Logger.getLogger(SMPPConstant.class);
  /**存放系统是否为调试状态*/
...
/**错误常量-正在登录*/
  ...
  //定义协议中用到Command_ID的值
    ...
    public static final int Bind_Transceiver_Command_Id = 9;
    public static final int Bind_Transceiver_Rep_Command_Id = 0x80000009;
	...
	static{
		try {
			resource = new Resource(new SMPPConstant().getClass(), "resource");
		} catch (IOException e) {
			e.printStackTrace();
		}
		initConstant();
