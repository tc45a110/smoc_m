...
public class SMGPSubmitMessage extends SMGPMessage
{
    ...
    public SMGPSubmitMessage(int msgType, int needReport, int priority, String serviceId, String feeType, String feeCode, String fixedFee, 
            ...
            String reserve,SMGPSubmitTLV tlv)
        throws IllegalArgumentException
    {
        ...
        int len = 126 + 21 * destTermId.length + msgContent.length + tlv.getBytes().length;
        buf = new byte[len];
        ...
        //增加tlv的字节数据
        loc = loc + 8;
        if(tlv.getBytes().length > 0){
        	System.arraycopy(tlv.getBytes(), 0, buf, loc, tlv.getBytes().length);
        }
