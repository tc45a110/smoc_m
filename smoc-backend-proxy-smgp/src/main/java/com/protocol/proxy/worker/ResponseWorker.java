...
public class ResponseWorker extends SuperMapWorker<Integer, TimerTask>{
	...
	private BlockingQueue<SMGPMessage> responseQueue;
	
	

	ResponseWorker(String channelID,String index,BlockingQueue<SMGPMessage> responseQueue) {
	...
	protected void doRun() throws Exception {
		SMGPMessage message = responseQueue.poll(FixedConstant.COMMON_POLL_INTERVAL_TIME,TimeUnit.SECONDS);
		if(message == null){
			return;
		}
		...
		SMGPSubmitRespMessage responseMessage = (SMGPSubmitRespMessage) message;
