...
public class BusinessWorker extends SuperCacheWorker{
	...
	public void doRun() throws Exception {
		...
		String accountExtendCode = AccountInfoManager.getInstance().getAccountExtendCode(accountID,businessRouteValue.getAccountSubmitSRCID());
		...
		if(businessRouteValue.getAccountMessageIDs().split(FixedConstant.SPLICER).length < businessRouteValue.getMessageTotal()){
			StringBuilder sb = new StringBuilder(businessRouteValue.getAccountMessageIDs());
			int messageIDNumber = businessRouteValue.getMessageTotal() - businessRouteValue.getAccountMessageIDs().split(FixedConstant.SPLICER).length; 
		...
		if(StringUtils.isEmpty(accountCarrier)){
			logger.error(
					new StringBuilder().append("发送运营商为空")
					.append("{}accountID={}")
					.append("{}phoneNumber={}")
		...
		if(FixedConstant.Carrier.INTL.name().equals(accountCarrier)){
			...
			areaCode = BusinessDictionaryManager.getInstance().getInternationalAreaCodeByPhoneNumber(businessRouteValue.getPhoneNumber());
			areaName = InternationalPhoneNumberUtil.getInternationalAreaName(businessRouteValue.getPhoneNumber());
			cityName = areaName;
		}else{
			//获取国内号段运营商
			segmentCarrier = BusinessDictionaryManager.getInstance().getInternalSegmentCarrier(businessRouteValue.getPhoneNumber());	
			//获取国内业务运营商
			businessCarrier = MNPUtil.getInternalBusinessCarrier(accountCarrier, businessRouteValue.getPhoneNumber(),segmentCarrier, AccountInfoManager.getInstance().getAccountTransferType(accountID));
		...
		if(StringUtils.isEmpty(businessCarrier)) {
			...
			logger.error(
					new StringBuilder().append("获取运营商失败")
					.append("{}accountID={}")
					.append("{}phoneNumber={}")
					.append("{}messageContent={}")
					.toString(),
		...
		if(StringUtils.isEmpty(areaCode)) {
			doLog(businessRouteValue);
			logger.error(
					new StringBuilder().append("省份路由失败")
					.append("{}accountID={}")
					.append("{}phoneNumber={}")
					.append("{}messageContent={}")
					.toString(),
		...
		//设置单价 和 统计当前消息的花费总金额
		String messagePrice = AccountPriceManager.getInstance().getPrice(accountID, segmentCarrier);
		if(StringUtils.isEmpty(messagePrice)) {
			doLog(businessRouteValue);
			logger.error(
					new StringBuilder().append("获取账号价格失败")
					.append("{}accountID={}")
					.append("{}phoneNumber={}")
					.append("{}messageContent={}")
					.toString(),
		...
		//当账号状态为非正常状态
		if(!FixedConstant.AccountStatus.NORMAL.name().equals(AccountInfoManager.getInstance().getAccountStatus(accountID))){
			doLog(businessRouteValue);
			logger.error(
					new StringBuilder().append("账号状态异常")
					.append("{}accountID={}")
					.append("{}phoneNumber={}")
					.append("{}messageContent={}")
					.toString(),
		...
		if( FixedConstant.AccountPayType.PREPAYMENT.name().equals(AccountInfoManager.getInstance().getPayType(accountID))){
			if(!AccountInfoManager.getInstance().isPositiveAvailableAmount(financeAccountID)){
				doLog(businessRouteValue);
				logger.error(
						new StringBuilder().append("余额不足")
						.append("{}accountID={}")
						.append("{}phoneNumber={}")
						.append("{}messageContent={}")
						.toString(),
		...
		logger.info(
				new StringBuilder().append("业务数据封装")
				.append("{}accountID={}")
				.append("{}phoneNumber={}")
				.append("{}messageContent={}")
				.append("{}耗时={}")
	...
	private void doLog(BusinessRouteValue businessRouteValue){
		...
		.append(FixedConstant.LOG_SEPARATOR)
		.append(StringUtils.defaultString(businessRouteValue.getAreaCode()))//10-区域编码
		.append(FixedConstant.LOG_SEPARATOR)
		.append(StringUtils.defaultString(businessRouteValue.getAreaName()))
		.append(FixedConstant.LOG_SEPARATOR)
		.append(StringUtils.defaultString(businessRouteValue.getCityName()))
		.append(FixedConstant.LOG_SEPARATOR)
		...
		.append(FixedConstant.LOG_SEPARATOR)
		.append(businessRouteValue.getMessageSignature())//18-签名
		.append(FixedConstant.LOG_SEPARATOR)
		.append(businessRouteValue.getMessageNumber())
		.append(FixedConstant.LOG_SEPARATOR)
		.append(businessRouteValue.getInfoType())
		.append(FixedConstant.LOG_SEPARATOR)
		.append(businessRouteValue.getIndustryTypes())//21-行业分类 
		.append(FixedConstant.LOG_SEPARATOR)
...
//		.append(businessRouteValue.getExecuteCheckCode())
		...
		.append(FixedConstant.LOG_SEPARATOR)
		.append(businessRouteValue.getPriceAreaCode())
