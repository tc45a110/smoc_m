...
public class AuthCheckerManager extends SuperMapWorker<String, AuthClient>{
	...
	public synchronized int authClient(String ip, String clientID, byte[] authClient, byte version, int timeStamp) {
		...
		CategoryLog.connectionLogger.debug(new StringBuilder(DateUtil.getCurDateTime())
				.append("匹配MD5:")
				.append("client={}")
				.append("{}source={}")
				.toString()
		...
		if (!result) {
			logger.warn("MD5匹配错误:client={}", clientID);
			return 21;
		}
	...
	public byte[] genAuthClient(String clientId, String sharedSecret,
			String timeStamp) {
		byte[] result = new byte[16];
		try {
			byte[] source = new byte[clientId.length() + sharedSecret.length()
					+ 7 + timeStamp.length()];
			System.arraycopy(clientId.getBytes(), 0, source, 0,
					clientId.length());
			System.arraycopy(sharedSecret.getBytes(), 0, source,
					clientId.length() + 7, sharedSecret.length());
			System.arraycopy(timeStamp.getBytes(), 0, source, clientId.length()
					+ 7 + sharedSecret.length(), timeStamp.length());
