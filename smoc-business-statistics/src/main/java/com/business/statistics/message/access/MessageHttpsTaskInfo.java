...
public class MessageHttpsTaskInfo {
	private static final Logger logger = LoggerFactory.getLogger(MessageHttpsTaskInfo.class);
	// 接入业务层一条mr日志字符串个数
				...
				try {
					...
					if (StringUtils.isNotEmpty(filePath) && StringUtils.isNotEmpty(fileName)) {
						...
						for(File file:fileList) {
							...
							worker  worker=new MessageHttpsTaskInfo().new worker(file, lineTime);
			...
			try {
				...
				List<BusinessRouteValue> businessRouteValuelist= getAccessBusinessMrData(file, lineTime);
					...
					for(BusinessRouteValue businessRouteValues: businessRouteValuelist) {
						AccessBusinessDao.updateMessageHttpsTaskInfo(businessRouteValues);
					}
